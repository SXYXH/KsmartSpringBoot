...
public class Examcontroller {
	...
	public String exam4(Model model) {
		List<Member> memberList = examService.getMemberList();
		Member memberInfo = examService.getMemberbyId("id004");
	...
	/**
	 * @ResponseBody 3: 반환하는 데이터 유형에 따라 text/html, application/json 형태로 변환후 응답해주는 어노테이션
	 * @return
	 */
	@GetMapping("/exam/api")
	@ResponseBody
	...
	/**
	 * @RequestParam(name = "요청시의 키" value= name의 별칭 , required = 요청시의 쿼리스트링 필수여부(true,false),defaultValue = "기본값") 쿼리스트링 바인딩하는 어노테이션 / 트루면 필수 펄스면 없어도 요청
	 * @param member -> 커맨드 객체(Controller 계층)
	 * @return
	 */
	@GetMapping("/exam/detail")
	public String detail(@RequestParam(name="memberId") String memberId
